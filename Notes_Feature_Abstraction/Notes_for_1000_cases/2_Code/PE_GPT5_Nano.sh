...
#SBATCH --job-name=PE_GPT5Nano_1000
...
python -u ./batch-abstract-notes-logged.py \
  ...
  --output ../3_Outputs/notes-for-1000-cases-pe-schema-gpt5nano.parquet \
  --note-col Text \
  --id-col EncounterCsn \
  ...
  --checkpoint-every 10 \
  --model gpt-5-nano \
  --json-out ../3_Outputs/notes-for-1000-cases-pe-schema-gpt5nano.json
