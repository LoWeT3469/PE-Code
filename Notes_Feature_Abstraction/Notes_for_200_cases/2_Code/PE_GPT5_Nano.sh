...
# keep it simple & match your shell behavior
set -e -o pipefail
...
# ---- Conda env ----
# (no `set -u` here; some site bashrcs choke under nounset)
source "$HOME/.bashrc"
conda activate PE
...
# >>> THIS is the key fix <<<
# many libs/tools read OPENAI_BASE_URL, not OPENAI_API_BASE
# mirror your CLI by setting it here if missing
...
python -u ./batch-abstract-notes-logged.py \
  ...
  --output ./notes-for-200-cases-18-features-gpt5-nano.parquet \
  --note-col Text \
  --id-col EncounterCsn \
  ...
  --var "exertional_symptoms:presence:Does the note indicate problems with 'exertion,' like exertional shortness of breath, or feeling weak with walking?" \
  ...
  --var "recent_or_active_malignancy:presence:Does the note indicate whether the patient has recent (prior 6 months) or active malignancy?" \
  ...
  --checkpoint-every 10 \
  --model gpt-5-nano \
  --json-out "./outputs/debug-${SLURM_JOB_ID:-local}-nano.json"
