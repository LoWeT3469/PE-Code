...
def create_llm_client(args):
    ...
    if provider in {"hf_local", "mistral_local"}:
        model_id = (
            args.model
            or os.environ.get("HF_MODEL_ID")
            or os.environ.get("MISTRAL_MODEL_DIR")
        )
        if not model_id:
            sys.exit("Missing model path/id. Set --model or HF_MODEL_ID or MISTRAL_MODEL_DIR.")

        use_4bit = os.environ.get(
            "HF_USE_4BIT",
            os.environ.get("MISTRAL_USE_4BIT", "0")
        ).strip() in {"1", "true", "True"}

        dtype_name = os.environ.get("HF_DTYPE", os.environ.get("MISTRAL_DTYPE", "bfloat16")).strip().lower()
        ...
        if use_4bit:
            if BitsAndBytesConfig is None:
                sys.exit("HF_USE_4BIT=1 but BitsAndBytesConfig is unavailable.")
    ...
    if provider not in {"azure", "openai"}:
        sys.exit(f"Unsupported --api-provider '{provider}'.")
...
def norm_presence(value: Any) -> Any:
    ...
    if s in {"present", "pos", "positive", "y", "yes", "true", "1"}:
        return "present"
    if s in {
        "absent", "explicitly absent", "neg", "negative", "n", "no",
        "false", "0", "denied", "denies"
    }:
        return "explicitly absent"
    if s in {
        "not mentioned", "none mentioned", "not stated", "unknown",
        "uncertain", "not documented", "no mention", "none"
    }:
...
def make_prompt(var_specs: List[str], explain_vars: Set[str], quote_per_var: bool):
    ...
    for spec in var_specs:
        ...
        if name in explain_vars:
            lines.append(
                ...
                "Provide 1–2 concise sentences (≤30 words) justifying the value, ideally including a brief quote (≤15 words) from the note."
            )
        if quote_per_var:
            lines.append(
    ...
    lines.extend([
        '- "conflict_flag": One of "yes","no","uncertain". Set to "yes" only when the note contains conflicting clinical information about the patient.',
        '- "conflict_explanation": If conflict_flag is "yes" or "uncertain", provide a non-empty explanation. If conflict_flag is "no", return null or "".',
    ])
...
def compute_justification_span(note: str, quote: str):
    if not quote:
        return None

    start = note.find(quote)
    if start == -1:
        q = normalize_ws(quote.strip())
        n = normalize_ws(note)
        idx = n.find(q)
        if idx == -1:
            return None
...
HEDGE_TERMS = ["uncertain", "possible", "suggest", "consider", "likely", "equivocal", "appears"]
LOW_CERTAINTY_VALUES = {"uncertain", "not mentioned", "unknown", "indeterminate", "equivocal"}
STRONG_TERMS = ["definite", "confirmed", "clear", "clearly", "no evidence of", "denies", "documented", "demonstrates"]
...
def main():
    ...
    ap.add_argument("--exp-all", action="store_true")
    ap.add_argument("--model", default=os.environ.get("HF_MODEL_ID", os.environ.get("GPT_MODEL", "gpt-5-nano")))
    ap.add_argument("--api-provider", choices=["azure", "openai", "mistral_local", "hf_local"])
    ...
    if provider in {"hf_local", "mistral_local"}:
        ...
        chunk_tokens = int(os.environ.get("HF_CHUNK_TOKENS", os.environ.get("MISTRAL_CHUNK_TOKENS", "512")))
        max_chunks = int(os.environ.get("HF_MAX_CHUNKS", os.environ.get("MISTRAL_MAX_CHUNKS", "0")))
        ...
        for idx, chunk in enumerate(chunk_text_by_tokens(tokenizer, note, chunk_tokens), start=1):
            if max_chunks and idx > max_chunks:
                sys.exit(f"Too many chunks ({idx - 1}); increase HF_CHUNK_TOKENS or set HF_MAX_CHUNKS=0.")
    ...
    for name in var_names:
        ...
        parsed[f"{name}_span"] = compute_justification_span(note, qval) if qval else None
